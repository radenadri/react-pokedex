...
const MultipleEvolution = ({evolutionDetail}) => {
  return (
    <div>
      ...
      <div className="flex items-center gap-4">
        ...
        <span>
          <Link to={`/pokemon/${evolutionDetail[2]?.id}`}>
            <img src={evolutionDetail[2]?.src} alt={evolutionDetail[2]?.src} />
          </Link>
          <p>{evolutionDetail[2]?.name}</p>
        </span>
      </div>
    </div>
  );
...
export default MultipleEvolution;